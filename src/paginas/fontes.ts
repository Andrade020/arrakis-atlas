...
export async function fontes(alvo: HTMLElement) {
  ...
  const [vidaIl, vermeMotion] = await Promise.all([
    fetch(import.meta.env.BASE_URL + "ilustracoes/vida.json")
      .then((r) => r.json()).catch(() => ({})),
    fetch(import.meta.env.BASE_URL + "ilustracoes/vida/verme_boca_motion.json")
      .then((r) => r.json()).catch(() => ({})),
  ]);
  const [aguaNova, tempestadeNova, fugaNova, regioesNovas] = await Promise.all(
    ["agua_novas.json", "tempestade_arrakis.json", "fuga_abertura.json", "regioes_detalhe.json"]
      .map((arquivo) => fetch(import.meta.env.BASE_URL + "ilustracoes/" + arquivo)
        .then((r) => r.json()).catch(() => ({})))
  );

  const en = emIngles();
  const ONDE = onde();
  const recentes: { nome: string; arquivo: string; prompt: string }[] = [
    ...(aguaNova.assets ?? []).map((v: any) => ({
      nome: `${prancha("agua")} · ${v.file === "cofre_agua_fremen.webp" ? tr("o cofre", "the vault") : tr("o orvalho", "the dew")}`,
      arquivo: v.file, prompt: v.prompt,
    })),
    ...(tempestadeNova.imagens ?? []).map((v: any) => ({
      nome: `${prancha("acessibilidade")} · ${v.arquivo?.includes("fremen") ? "fremen" : tr("Império", "Empire")}`,
      arquivo: v.arquivo, prompt: v.prompt,
    })),
    ...(fugaNova.arquivo ? [{ nome: prancha("fuga"), arquivo: fugaNova.arquivo, prompt: fugaNova.prompt }] : []),
    ...Object.entries(regioesNovas as Record<string, any>).map(([cod, v]) => ({
      nome: ONDE[cod] ?? cod, arquivo: String(v.arquivo ?? "").replace(/^ilustracoes\//, ""), prompt: v.prompt,
    })),
  ].filter((v) => v.arquivo && v.prompt);
  ...
  alvo.innerHTML = `<article class="folha">
    ...
    <div class="corpo">
      ...
      <p>${tr(`Aparecem só na página de comparação. Em cada par fica dito se o número de
      Arrakis foi medido ou simulado.`, `These appear only on the comparison page. Each pair says whether the
      ...
      feito para cada uma.`, `The original panoramas on the regions page and the opening card images were
      ...
      <p class="nota">${tr(`Essas estampas originais usam o mesmo pedido de estilo (<em>fotografia
      ...
      modelo é`, `These original plates use the same style request (<em>large-format monochrome expedition
      ...
      <p>${tr(`As novas vistas da água, as duas perspectivas da tempestade, o prólogo da fuga e os detalhes das sete regiões também foram gerados com a ferramenta integrada <code>image_gen</code>. São interpretações visuais; os movimentos do site foram construídos sobre as imagens e os mapas por código. Cada pedido completo e a imagem final podem ser vistos abaixo.`,
        `The new water views, two storm perspectives, escape prologue and seven regional details were also generated with the built-in <code>image_gen</code> tool. They are visual interpretations; the site's motion was built over the images and maps in code. Each full request and final image can be viewed below.`)}</p>
      ${recentes.map((v) => `<details class="pedido-corte"><summary>${esc(v.nome)} · ${esc(v.arquivo)}</summary>
        <p><a class="link" href="${import.meta.env.BASE_URL}ilustracoes/${esc(v.arquivo)}">${tr("Abrir imagem", "Open image")}</a></p>
        <p class="nota">${esc(v.prompt)}</p></details>`).join("")}

      <p>${tr(`As imagens das páginas ${plac("verme")} e ${plac("vida")} (o verme, os bichos
      ...
      `The pictures on pages ${plac("verme")} and ${plac("vida")} (the worm, the animals and the plants)
      ...
      <p class="nota">${tr("O movimento da boca do verme na Página 04 foi criado a partir da estampa já publicada, com câmera fixa, em um clipe de cinco segundos de",
        "The worm mouth motion on Page 04 was created from the previously published image, with a fixed camera, as a five-second clip by")}
        <code>bytedance/seedance-1-lite</code>.</p>
      ${vermeMotion.prompt ? `<details class="pedido-corte"><summary>${tr("Ler o pedido do movimento do verme", "Read the worm motion request")}</summary>
        <p><a class="link" href="${import.meta.env.BASE_URL}${esc(vermeMotion.arquivo)}">${tr("Abrir clipe", "Open clip")}</a></p>
        <p class="nota">${esc(vermeMotion.prompt)}</p></details>` : ""}
